...
class RgbIntrinsicsPage(QWidget):
    def __init__(self, state: SessionState, on_complete_changed):
        ...
        self.title = QLabel("Step 1 — Load RGB intrinsics")
        ...
        self.load_info = QLabel("Load RGB intrinsics as a 3×3 matrix JSON.")
        self.btn_load = QPushButton("Load RGB intrinsics (JSON)")
        ...
        self.summary_title = QLabel("RGB intrinsics loaded:")
        self.summary_mat = QLabel("")
        self.summary_mat.setTextInteractionFlags(self.summary_mat.textInteractionFlags() | self.summary_mat.textInteractionFlags())
        self.summary_mat.setStyleSheet("font-family: Consolas, Menlo, Monaco, monospace;")

        self.btn_reload = QPushButton("Load different RGB intrinsics…")
    ...
    def refresh(self):
        if self.is_complete():
            self.summary_mat.setText(format_K(self.state.K_rgb))
            self.stack.setCurrentIndex(1)  # Summary
        else:
            self.stack.setCurrentIndex(0)  # Load

    def load_intrinsics(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select RGB intrinsics JSON", "", "JSON Files (*.json)"
        )
        if not path:
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Failed to load RGB intrinsics", str(e))
