...
class PlaneFittingPage(LiveImagePage):
    ...
    def __init__(self, state: SessionState, on_complete_changed=None, parent=None):
        ...
        self._live_depth = None  # rs.depth_frame
        ...
        self._snap_depth = None  # rs.depth_frame
        ...
        # RealSense depth tuning (local only)
        ...
        # If we already have a confirmed plane result in state, we still start idle
        # (Wizard gating should use state.has_plane_confirmed / state.plane_confirmed)
        # so we do NOT need a special 'frozen' init state here.
        self._mode = "idle"

        # If state already has valid outputs, show them in stats immediately
        if self.state.has_plane_confirmed:
            # Keep display idle, but show last known stats placeholders as "done"
            # (we don't recompute anything; we just reflect completion)
            self._stats = [
                "Inliers: (done)",
                "Mean: (done)",
                "Median: (done)",
    ...
    def _build_controls(self) -> None:
        ...
        # Toggle: depth tuning (STARTS OFF)
        ...
        # ---- Layout: Start left, Toggles stacked right ----
        wrap = QWidget()
    ...
    def get_frame(self) -> np.ndarray | None:
        ...
        # Update depth caches whenever depth exists (independent from color)
        if df:
            self._live_depth = df
    ...
    def _apply_depth_defaults(self) -> None:
        ...
        # Snapshot previous ONCE (so restore is precise)
        ...
        # Emitter: prefer AUTO (often 2), else ON
        ...
        # Visual preset back to 0 (clamped)
    ...
    def start_clicked(self) -> None:
        ...
        try:
            ...
            # Apply tuning only if user enabled it
            if self.use_tuned_depth_settings:
                self._apply_depth_tuning()
        ...
        # IMPORTANT: only touch existing SessionState fields
    ...
    def _fit_and_grid(self) -> None:
        ...
        # Grid in camera frame (ONLY persist what SessionState already defines)
    ...
    def _update_buttons(self) -> None:
        # Start only when idle (no stop/redo in this page by design)
