...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.runtime_list.setDisabled(True)  # display only
        ...
        self.pages.addWidget(self.page_ptr_depth)  # index 5
        self.pages.addWidget(self.page_overlay_preview)  # index 6
    ...
    def step_complete(self, idx: int) -> bool:
        if idx == self.IDX_XRAY:  # X-ray intrinsics
            return self.state.has_xray_intrinsics
        if idx == self.IDX_CAM_CALIB:  # camera calibration
            return self.state.has_rgb_intrinsics
        if idx == self.IDX_PLANE:  # plane fitting
            return self.state.has_plane_confirmed
        if idx == self.IDX_MARKERS:  # marker selection
            return self.state.has_xray_points_confirmed
        if idx == self.IDX_CAM2XRAY:  # camera -> xray (PnP)
            return self.state.has_cam_to_xray
        if idx == self.IDX_PTR_DEPTH:  # Mode B: Pointer Tool Depth
            return self.state.has_d_x
        if idx == self.IDX_OVERLAY_PREVIEW:
            return self.state.has_H_xc
        return False
    ...
    def go_to(self, idx: int) -> None:
        ...
        # ----------------------------
        # Mode A locked after entering Mode B
        # ----------------------------
        if self.mode_b_active and target_in_a:
            return
    
        # ----------------------------
        # Navigation within / into Mode A
        # ----------------------------
        if target_in_a:
            # forward navigation in Mode A is gated
        ...
        # ----------------------------
        # Entering / navigating Mode B
        # ----------------------------
        elif target_in_b:
            # first entry into Mode B is only allowed from last Mode-A page
