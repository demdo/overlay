...
def show_blob_error_histogram(
    ...
    """
    Histogram for blob errors (euclidean pixel error), styled like your RANSAC histogram.

    - left of P95: blue
    - right of P95: red (tail)
    - dashed P95 marker at EXACT e_p95
    - stats-only legend in upper-right
    """
    ...
    # split by P95 (bar centers)
    is_left = centers <= e_p95
    is_right = ~is_left
    ...
    # bars: FIX colors to match your RANSAC plot
    ax.bar(
        lefts[is_left], counts[is_left], width=widths[is_left],
        align="edge", alpha=0.85, color="#1f77b4", edgecolor="none"
    )
    ax.bar(
        lefts[is_right], counts[is_right], width=widths[is_right],
    ...
    # --- P95 marker: EXACT x position, stable height ---
    k = int(np.clip(np.searchsorted(edges, e_p95, side="right") - 1,
                    0, len(counts) - 1))

    x_left = lefts[k]            # LEFT EDGE of bin
    bar_h  = float(counts[k])    # height of that bin

    # Draw slightly above bar
    y0 = 0.05 * bar_h            # small offset from bottom
    y1 = bar_h * 1.08 + 0.5      # just slightly above bar

    ax.plot([x_left, x_left], [y0, y1],
            linestyle="--", linewidth=2.5, color="black")

    ax.text(x_left + 0.02 * xmax, y1,
            r"$P_{95}$", ha="left", va="bottom")

    # stats-only legend (upper-right)
